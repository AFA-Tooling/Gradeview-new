...
const weightedSectorDonutPlugin = {
  ...
  afterDatasetsDraw(chart) {
    ...
    arcs.forEach((arc, i) => {
      ...
      const earnedFraction = seg.earned == null || seg.cap <= 0
        ? null
        : Math.max(0, Math.min(1, seg.earned / seg.cap));
      const earnedEndAngle = drawStartAngle + drawSpan * (earnedFraction ?? 0);
      ...
      if (earnedFraction != null && earnedFraction > 0) {
        ctx.save();
        ctx.globalAlpha = alphaScale;
        ctx.beginPath();
      ...
      if (earnedFraction != null && earnedFraction > 0 && earnedFraction < 1) {
        ctx.save();
        ctx.beginPath();
        ctx.moveTo(
...
function formatRawScore(value) {
  return formatContractPoints(value);
}

function formatDate(dateString) {
  return formatCourseDateTime(dateString);
}
      ...
      tooltip: {
        ...
        callbacks: {
          label: function (context) {
            ...
            const points = Number.isFinite(cap) ? Math.min(cap, (pct / 100) * cap) : null;
            return `${context.dataset.label}: ${pct.toFixed(2)}% (${formatContractPoints(points)}/${formatContractPoints(cap)})`;
          },
        },
      },
    },
...
const EvidenceStatusChip = memo(function EvidenceStatusChip({ row }) {
  ...
  const tone = meta.tone === 'error'
    ...
    : meta.tone === 'warning'
      ...
      : meta.tone === 'success'
        ? { color: '#0F766E', backgroundColor: '#ECFDF5' }
        : { color: chartColors.muted, backgroundColor: chartColors.band };
  return <Chip size="small" label={meta.label} sx={{ ...tone, fontWeight: 700 }} />;
});
...
function StudentProfileContent({ studentData, hideTopSnapshot = false }) {
  ...
  const assignmentEvidence = useMemo(() => getAssignmentEvidence(studentData), [studentData]);
  const categoryBlocks = useMemo(() => buildCategoryPresentations(studentData).map((block) => {
    if (block.type !== 'exam') return block;
    const rows = getExamRows(studentData, block.key);
    return {
      ...block,
      exam: {
        attempts: rows.length,
        questionBestAvailable: rows.some((row) => optionalNumber(row.questionBestPercentage) != null),
        positiveClobberCount: getActualClobberRows(rows).length,
      },
    };
  }), [studentData]);
  ...
  const displayCategoriesData = useMemo(() => (
    Object.fromEntries(
      categoryBlocks
        .filter((block) => block?.cap != null && block.cap > 0)
        .map((block) => [block.label, {
          key: block.key,
          basis: block.basis,
          total: block.exactScore,
          exactScore: block.exactScore,
          capPoints: block.cap,
          maxPoints: block.cap,
          percentage: block.percentage,
          status: block.canonicalStatus,
          source: block.source,
        }])
    )
  ), [categoryBlocks]);

  const radarData = useMemo(() => categoryBlocks
    .filter((block) => block?.percentage != null && block?.exactScore != null && block?.cap != null)
    .map((block) => ({
      category: block.label,
      percentage: block.percentage,
      score: block.exactScore,
      maxPoints: block.cap,
    })), [categoryBlocks]);

  const currentGrade = useMemo(() => {
    if (!standing.letter) return null;
    return {
      grade: standing.letter,
      range: standing.bin?.range || '',
      roundedTotal: standing.displayScore,
    };
  }, [standing]);
  ...
  const categorySnapshot = useMemo(() => {
    const get = (categoryName) => {
      ...
      const score = optionalNumber(category?.exactScore);
      const cap = optionalNumber(category?.capPoints ?? category?.maxPoints);
      if (score == null || cap == null || cap <= 0) {
        return null;
      }
  ...
  const examComponentTrends = useMemo(() => {
    ...
    return {
      quest: normalizeTrend(getExamTrend(studentData, 'quest')),
      midterm: normalizeTrend(getExamTrend(studentData, 'midterm')),
      postterm: normalizeTrend(getExamTrend(studentData, 'postterm')),
    };
  }, [studentData]);
  ...
  const overallCategoryDonut = useMemo(() => {
    ...
    validEntries.forEach(([category, data], index) => {
      ...
      const exactScore = optionalNumber(data?.exactScore);
      const earned = exactScore == null ? null : Math.max(0, Math.min(cap, exactScore));
      ...
      segmentMeta.push({
        ...
        cap,
        earned,
        remaining: earned == null ? null : Math.max(0, cap - earned),
        type: 'category',
        earnedFraction: earned == null || cap <= 0 ? null : earned / cap,
        weightPercentage: totalCap > 0 ? (cap / totalCap) * 100 : 0,
        canonicalStatus: data?.status || 'unavailable',
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: function(context) {
            ...
            const meta = overallCategoryDonut.segmentMeta[idx];
            if (!meta) return '';
            const pct = meta.earned == null || meta.cap <= 0 ? null : (meta.earned / meta.cap) * 100;
            return [
              `Earned : ${formatContractPoints(meta.earned)} / ${formatContractPoints(meta.cap)}`,
              `Score  : ${formatContractPercentage(pct, 2)}`,
            ];
          },
        },
      },
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: function(context) {
            ...
            return `Score: ${formatContractPercentage(context.parsed.r)} (${formatContractPoints(data.score)}/${formatContractPoints(data.maxPoints)})`;
          },
        },
      },
      datalabels: {
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: function(context) {
            ...
            if (!data) return '';
            let label = `Raw: ${formatRawScore(data.score)} / ${formatRawScore(data.maxPoints)} (${formatContractPercentage(data.percentage, 2)})`;
  ...
  return (
    <Box>
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ xs: 'flex-start', md: 'center' }} justifyContent="space-between">
            <Box>
              ...
              <Typography variant="h5" sx={{ color: chartColors.ink, fontWeight: 750, lineHeight: 1.15 }}>
                {standing.exactScore == null || standing.cap == null
                  ? 'Unavailable'
                  : `${formatContractPoints(standing.exactScore)} / ${formatContractPoints(standing.cap)}`}
              ...
              {['Quest', 'Midterm', 'Postterm'].map((category) => {
                ...
                return (
                  <Chip
                    key={category}
                    size="small"
                    label={`${category}: ${item ? `${formatContractPoints(item.score)}/${formatContractPoints(item.cap)}` : 'Unavailable'}`}
      ...
      <Grid container spacing={3} sx={{ mb: 3, alignItems: 'stretch' }}>
        {/* Overall Summary */}
        <Grid item xs={12} md={12} lg={5} sx={{ display: 'flex', minWidth: 0 }}>
          <Paper
            elevation={0}
            sx={{
              ...cardSx,
                ...
                {overallCategoryDonut.values.length === 0 ? (
                  ...
                  <>
                    ...
                    >
                      ...
                      >
                        {formatContractPoints(standing.exactScore)}
                      </Typography>
                      <Typography variant="body2" sx={{ color: chartColors.softText, fontSize: 17, mt: 0.6 }}>
                        / {formatContractPoints(standing.cap)}
                      </Typography>
                      {currentGrade && (
                        <Box
                          sx={{
                ...
                {overallCategoryDonut.segmentMeta.map((segment) => (
                  ...
                  >
                    ...
                    <Typography variant="caption" sx={{ color: chartColors.muted, fontWeight: 650, whiteSpace: 'nowrap' }}>
                      {formatContractPoints(segment.earned)}/{formatContractPoints(segment.cap)} · {Math.round(segment.weightPercentage)}%
                    </Typography>
                  </Box>
                ))}
              </Box>
        ...
        <Grid item xs={12} md={12} lg={7} sx={{ display: 'flex', minWidth: 0 }}>
          <Paper
            elevation={0}
            sx={{
              ...cardSx,
          ...
          >
            ...
            <TableContainer sx={{ mt: 2, borderRadius: 2, overflowX: 'auto', overflowY: 'hidden' }}>
              <Table size="small" sx={{ '& th, & td': { px: { xs: 1, xl: 2 } } }}>
                <TableHead>
                  ...
                  {Object.entries(displayCategoriesData).map(([category, data]) => {
                    return (
                      <TableRow key={category} hover>
                        <TableCell><strong>{category}</strong></TableCell>
                        <TableCell align="center">{formatContractPoints(data.exactScore)}</TableCell>
                        <TableCell align="center">{formatContractPoints(data.capPoints ?? data.maxPoints)}</TableCell>
                        <TableCell align="center"><ProgressBattery value={data.percentage} /></TableCell>
                        <TableCell align="center">
                          <Chip
                            size="small"
                            label={data.status === 'available' || data.status === 'complete' || data.status === 'legacy'
                              ? 'Final policy'
                              : String(data.status || 'unavailable').replace(/_/g, ' ').replace(/^\w/, (letter) => letter.toUpperCase())}
                            sx={{ fontWeight: 700, color: chartColors.muted }}
                          />
                        </TableCell>
                      </TableRow>
                    );
              ...
              <Box sx={{ mb: 2 }}>
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} alignItems={{ xs: 'flex-start', sm: 'center' }} justifyContent="space-between">
                  ...
                  <Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap>
                    <Chip
                      size="small"
                      label={`Topic best ${formatContractPoints(card.topicScore)} / ${formatContractPoints(card.topicCap || card.cap)}`}
                      sx={{ fontWeight: 700, color: '#0F766E', backgroundColor: '#ECFDF5' }}
                    />
                    <Chip
                      size="small"
                      label={`Final ${card.finalSnapshot ? `${formatContractPoints(card.finalSnapshot.score)} / ${formatContractPoints(card.finalSnapshot.cap)}` : 'Unavailable'}`}
          ...
          <Table size="small" stickyHeader>
            ...
            <TableBody>
              {sortedAssignments.map((assignment, idx) => {
                return (
                  <TableRow key={assignment.assignmentId || `${assignment.category}-${assignment.name}-${idx}`} data-assignment-id={assignment.assignmentId} hover>
